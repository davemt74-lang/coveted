...
$withoutComments = static function (string $content): string {
    $result = '';
...
$accessCode = $withoutComments($access);
$businessCode = $withoutComments($business);
$rewardsCode = $withoutComments($rewards);
$campaignsCode = $withoutComments($campaigns);
$servicePackagesCode = $withoutComments($servicePackages);
...
$missing($accessCode, 'partner_pro', 'enforcement must not depend on the Partner Pro package key');
$missing($accessCode, "provider='stripe'", 'authorization helpers must not use Stripe state directly');
$missing($accessCode, 'billing_subscriptions', 'authorization helpers must not query subscription tables directly');
...
$contains($business, 'if (!$workspaceAllowed && !$isSystemAdmin)', 'unentitled partners must be stopped before operational business data loads');
...
$missing($businessCode, "package_key === 'partner_pro'", 'business workspace must not gate on package names/keys');
$missing($businessCode, 'travel.planning', 'future travel hooks must not surface in the business workspace');
...
$contains($rewards, 'Your current partner package does not include business offer management.', 'business claim refunds must require partner offers at the shared service boundary');
$contains($campaigns, '$actor, $ownerId, \'partner.campaigns\'', 'business campaign creation/mutation must require partner campaigns');
$missing($rewardsCode, 'partner_pro', 'reward service must not know package keys');
$missing($campaignsCode, 'partner_pro', 'campaign service must not know package keys');
...
$missing($servicePackagesCode, "package_key === 'partner_pro'", 'Admin activation must not depend on a package key');
...
foreach ([$accessCode, $businessCode, $rewardsCode, $campaignsCode, $servicePackagesCode] as $content) {
