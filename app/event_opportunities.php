...
function coveted_event_opportunity_active_perks(PDO $pdo): array
{
    ...
    $result = [];
    foreach ($rows as $row) {
        $result[(string)$row['relationship_key']] = (int)$row['active_perks'];
    }
    return $result;
}
...
function coveted_event_opportunity_active_campaigns(PDO $pdo): array
{
    ...
    $result = [];
    foreach ($rows as $row) {
        $result[(string)$row['relationship_key']] = (int)$row['active_campaigns'];
    }
    return $result;
}
...
function coveted_event_opportunities(array $admin, ?PDO $pdo = null): array
{
    ...
    $rows = $pdo->query(
        "SELECT
            ...
            (SELECT COUNT(*)
             ...
             WHERE gm.group_id = g.id AND gm.membership_status = 'active') AS active_members,
            (SELECT COUNT(*)
             FROM events e2
             JOIN event_locations el2 ON el2.event_id = e2.id AND el2.location_id = l.id
             WHERE e2.group_id = g.id AND e2.status = 'completed') AS completed_events,
            (SELECT COUNT(*)
             FROM events e3
             JOIN event_locations el3 ON el3.event_id = e3.id AND el3.location_id = l.id
             WHERE e3.group_id = g.id
               AND e3.status IN ('published','closed')
               AND COALESCE(e3.ends_at, e3.starts_at) >= NOW()) AS upcoming_at_location,
            (SELECT COUNT(*)
             FROM events e4
             WHERE e4.group_id = g.id
               AND e4.status IN ('published','closed')
               AND COALESCE(e4.ends_at, e4.starts_at) >= NOW()) AS upcoming_for_group,
            (SELECT MAX(e5.starts_at)
             FROM events e5
             JOIN event_locations el5 ON el5.event_id = e5.id AND el5.location_id = l.id
             WHERE e5.group_id = g.id AND e5.status = 'completed') AS last_completed_at,
            (SELECT COUNT(*)
             FROM event_attendance ea
             JOIN events ae ON ae.id = ea.event_id AND ae.group_id = g.id AND ae.status = 'completed'
             JOIN event_locations ael ON ael.event_id = ae.id AND ael.location_id = l.id
             WHERE ea.status IN ('checked_in','attended','left_early')) AS verified_visits,
    ...
    $activePerks = coveted_event_opportunity_active_perks($pdo);
    $activeCampaigns = coveted_event_opportunity_active_campaigns($pdo);
    $statusWeight = [
        'event_venue' => 4,
        'partner' => 9,
        'preferred_partner' => 14,
        'home_venue' => 19,
    ];
    ...
    foreach ($rows as $row) {
        ...
        $relationshipKey = (int)$row['group_id'] . ':' . (int)$row['location_id'];
        $upcomingForGroup = (int)$row['upcoming_for_group'];
        $completedEvents = (int)$row['completed_events'];
        $verifiedVisits = (int)$row['verified_visits'];
        ...
        if (!empty($row['last_completed_at'])) {
            try {
                $last = coveted_utc_datetime((string)$row['last_completed_at']);
                $daysSince = max(0, (int)floor(($now->getTimestamp() - $last->getTimestamp()) / 86400));
            } catch (Throwable) {
                $daysSince = 60;
            }
        }
        ...
        $score += $completedEvents >= 2 ? 7 : ($completedEvents > 0 ? 3 : 0);
        $score += $verifiedVisits >= 20 ? 5 : ($verifiedVisits >= 8 ? 3 : 0);
        $score += $activeMembers >= 12 ? 4 : 0;
        ...
        $mystery = !empty($row['mystery_events_enabled']) && $completedEvents >= 1;
        $eventType = $mystery ? 'mystery' : 'regular';
        $locationVisibility = $mystery ? 'scheduled_reveal' : 'immediate';
        $capacityBase = $completedEvents > 0
            ? (int)round($verifiedVisits / max(1, $completedEvents)) + 4
            : min(20, max(10, $activeMembers));
        $capacity = max(10, min(36, $capacityBase));
        $timezone = trim((string)($row['location_timezone'] ?? '')) ?: 'America/Phoenix';
        $start = coveted_event_opportunity_suggested_start($timezone, $daysSince >= 28 ? 10 : 14);
        $end = coveted_utc_datetime($start)->modify('+2 hours 30 minutes')->format('Y-m-d H:i:s');

        $title = $mystery
            ? (string)$row['group_name'] . ' Mystery Night'
            : (string)$row['group_name'] . ' at ' . (string)$row['location_name'];
        ...
        if ($perks > 0) {
            $evidenceParts[] = $perks . ' active standing perk' . ($perks === 1 ? '' : 's');
        }
        if ($campaigns > 0) {
            $evidenceParts[] = $campaigns . ' active partner campaign' . ($campaigns === 1 ? '' : 's');
        }
        if (!empty($row['benefits_enabled'])) {
            $evidenceParts[] = 'relationship benefits enabled';
        }

        $items[] = [
            'key' => coveted_event_opportunity_key($groupRef, $locationRef),
            'score' => $score,
            'priority' => $score >= 78 ? 1 : ($score >= 62 ? 2 : 3),
            'category' => 'Events',
            'title' => 'Plan ' . $title,
            'detail' => 'This group has no future event scheduled and has a proven venue relationship that can support the next gathering.',
            'evidence' => implode(' · ', $evidenceParts) . '.',
            'href' => '/admin/event-opportunities.php?opportunity=' . rawurlencode(coveted_event_opportunity_key($groupRef, $locationRef)),
            ...
            'suggested_draft' => [
                ...
                'business_ref' => (string)$row['business_ref'],
                'title' => $title,
                'description' => 'Recommended from Coveted relationship, attendance and event-cadence signals. Review all details before publishing.',
    ...
    usort($items, static function (array $a, array $b): int {
        ...
        return $score !== 0 ? $score : strcmp((string)$a['key'], (string)$b['key']);
    });
...
function coveted_event_opportunity_by_key(array $admin, string $key, ?PDO $pdo = null): ?array
{
    ...
    foreach (coveted_event_opportunities($admin, $pdo) as $item) {
        if (hash_equals((string)$item['key'], $key)) {
            return $item;
        }
    }
    return null;
...
function coveted_event_opportunity_create_draft(array $admin, string $key, ?PDO $pdo = null): array
{
    ...
    $draft = (array)$item['suggested_draft'];
    $created = coveted_event_create($admin, (int)$draft['group_id'], $draft);
    try {
    ...
    coveted_audit(
        'event.opportunity_draft_created',
        'event',
        (string)$created['public_id'],
        [
            'opportunity_key' => (string)$item['key'],
            'group_ref' => (string)$draft['group_ref'],
            'location_ref' => (string)$draft['location_ref'],
...
function coveted_event_opportunity_agent_context(array $admin, ?PDO $pdo = null): array
{
    ...
    return [
        'total' => count($items),
        'high_priority' => count(array_filter($items, static fn(array $row): bool => (int)$row['priority'] === 1)),
        'recommendations' => array_slice($items, 0, 12),
        'authority' => 'Recommendations are read-only. Event configuration and creation remain Coveted System Admin authority; Agent actions may use only the existing allowlisted canonical create_event action when autonomous mode and user intent permit it.',
    ];
}
