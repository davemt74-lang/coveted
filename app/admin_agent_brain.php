 ...
 * becomes complete. Full System Sample Mode swaps this read layer for the
 * canonical in-memory synthetic system pack; mutators remain disabled there.
 */
...
function coveted_admin_agent_snapshot(array $admin, ?PDO $pdo = null): array
{
    ...
    if (coveted_system_sample_mode($admin, $pdo)) {
        try {
            $sampleProviders = coveted_ai_provider_statuses($pdo);
        } catch (Throwable) {
            $sampleProviders = [];
        }
        $providerStatuses = array_map(static fn(array $provider): array => [
            'provider' => (string)($provider['provider'] ?? ''),
            'configured' => !empty($provider['configured']),
            'enabled' => !empty($provider['enabled']),
            'model' => (string)($provider['model'] ?? ''),
        ], $sampleProviders);
        return coveted_system_sample_agent_snapshot($admin, $providerStatuses);
    }
...
function coveted_admin_agent_context_message(array $snapshot): string
{
    ...
    $sourceGuidance = !empty($snapshot['sample_mode'])
        ? "This block is generated from Coveted's canonical synthetic Full System Sample pack. Every business, person, event, CRM record, relationship, reward, claim and Agent signal in it is preview-only data. Treat it as internally consistent read-only demo state. Never describe a sample entity as live production state and never imply that a mutation against a sample reference was executed."
        : "This block is generated by Coveted from canonical database/services, not typed by the administrator. Treat it as read-only current application state and historical audit memory.";

    return "SERVER-SUPPLIED COVETED ADMIN CONTEXT\n"
        . $sourceGuidance
        . " Do not invent relationships that are not represented here or in Coveted's capability catalog. When recommending work, prioritize listed opportunities and explain the underlying evidence. You may direct the administrator to the provided internal Admin routes, but do not claim you executed a mutation unless a server tool explicitly confirms it.\n\n"
        . $json;
}
