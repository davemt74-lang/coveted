...
{
    $ref = trim($ref);
    if ($ref === '' || strlen($ref) > 64 || preg_match('/^[A-Za-z0-9_-]+$/', $ref) !== 1) throw new InvalidArgumentException('Invalid Admin Agent task reference.');
    return $ref;
}
...
function coveted_admin_agent_task_by_ref(array $admin, string $ref, ?PDO $pdo = null): ?array
{
    ...
    try {
        $stmt = $pdo->prepare('SELECT * FROM admin_agent_tasks WHERE public_id = ? AND owner_user_id = ? LIMIT 1');
...
function coveted_admin_agent_tasks_list(array $admin, string $status = 'active', int $limit = 100, ?PDO $pdo = null): array
{
    ...
    $where = 'owner_user_id = ?'; $params = [(int)$admin['id']];
    if ($status === 'active') $where .= " AND status IN ('suggested','approved','in_progress')";
    elseif ($status !== 'all') { $where .= ' AND status = ?'; $params[] = $status; }
    try {
        $stmt = $pdo->prepare(
            "SELECT id,public_id,title,detail,priority,status,source_type,source_key,source_href,completed_at,created_at,updated_at
             FROM admin_agent_tasks WHERE {$where}
...
function coveted_admin_agent_task_create_manual(array $admin, string $title, string $detail = '', int $priority = 2, ?PDO $pdo = null): array
{
    coveted_admin_agent_tasks_require_admin($admin);
    ...
    )->execute([$publicId,(int)$admin['id'],$title,$detail !== '' ? $detail : null,$priority,(int)$admin['id'],(int)$admin['id']]);
...
function coveted_admin_agent_tasks_sync_opportunities(array $admin, array $opportunities, ?PDO $pdo = null): array
{
    ...
    foreach (array_slice(array_values($opportunities),0,20) as $item) {
        ...
        $find = $pdo->prepare("SELECT id,public_id,status FROM admin_agent_tasks WHERE owner_user_id = ? AND source_type = 'opportunity' AND source_key = ? LIMIT 1");
        $find->execute([(int)$admin['id'],$key]);
        $existing = $find->fetch();
        if ($existing) {
            if (in_array((string)$existing['status'],['completed','dismissed'],true)) { $skipped++; continue; }
            $pdo->prepare('UPDATE admin_agent_tasks SET title=?,detail=?,priority=?,source_href=?,updated_by_user_id=?,updated_at=UTC_TIMESTAMP() WHERE id=?')
...
function coveted_admin_agent_task_set_status(array $admin, string $ref, string $status, ?PDO $pdo = null): void
{
    ...
    if (!in_array($status,coveted_admin_agent_task_allowed_transitions($previous),true)) throw new InvalidArgumentException('That task status transition is not allowed.');
    $completedAt = $status === 'completed' ? 'UTC_TIMESTAMP()' : 'NULL';
    $pdo->prepare("UPDATE admin_agent_tasks SET status=?,updated_by_user_id=?,completed_at={$completedAt},updated_at=UTC_TIMESTAMP() WHERE id=?")
        ->execute([$status,(int)$admin['id'],(int)$task['id']]);
...
function coveted_admin_agent_tasks_context(array $admin, ?PDO $pdo = null): array
{
    coveted_admin_agent_tasks_require_admin($admin);
    $pdo ??= coveted_db();
    if (!coveted_admin_agent_tasks_schema_available($pdo)) return ['available'=>false,'active_total'=>0,'active_tasks'=>[],'route'=>'/admin/agent-tasks.php'];
    ...
    return [
        ...
        'active_tasks'=>array_map(static fn(array $task): array => [
            'task_ref'=>(string)$task['public_id'],'title'=>(string)$task['title'],'priority'=>(int)$task['priority'],
            'status'=>(string)$task['status'],'source_type'=>(string)$task['source_type'],'source_href'=>(string)($task['source_href'] ?? ''),
        ],$active),
...
function coveted_admin_agent_tasks_context_current(?PDO $pdo = null): array
{
    ...
    if (!$admin || !coveted_is_system_admin($admin)) return ['available'=>false,'active_total'=>0,'active_tasks'=>[],'route'=>'/admin/agent-tasks.php'];
