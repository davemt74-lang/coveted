...
function coveted_site_logo_asset(): ?array
{
    ...
    foreach ($types as $filename => $mime) {
        ...
        if (!is_array($image) || strtolower((string)($image['mime'] ?? '')) !== $mime) continue;
        return [
            'filename' => $filename,
            'path' => $path,
...
function coveted_site_logo_validate_upload(array $file): array
{
    ...
    if ($tmp === '' || $bytes < 1 || $bytes > 5 * 1024 * 1024) throw new InvalidArgumentException('The logo image must be between 1 byte and 5 MB.');
    $image = @getimagesize($tmp);
    if (!is_array($image)) throw new InvalidArgumentException('Upload a valid PNG, WebP or JPEG image.');
    ...
    if ($extension === '') throw new InvalidArgumentException('Logo artwork must be PNG, WebP or JPEG. SVG is not accepted for uploaded branding.');
    $width = (int)$image[0];
    $height = (int)$image[1];
    if ($width < 32 || $height < 16 || $width > 5000 || $height > 5000) throw new InvalidArgumentException('Logo dimensions must be between 32×16 and 5000×5000 pixels.');
    return ['tmp_name'=>$tmp,'extension'=>$extension,'mime_type'=>$mime,'width'=>$width,'height'=>$height,'bytes'=>$bytes];
}
...
function coveted_site_logo_store(array $admin, array $file): array
{
    ...
    $directory = coveted_site_branding_dir();
    if (!is_dir($directory) && !mkdir($directory, 0755, true) && !is_dir($directory)) throw new RuntimeException('Unable to create the site branding upload directory.');
    ...
    $moved = is_uploaded_file($validated['tmp_name'])
        ...
        : ((coveted_config('app')['environment'] ?? 'production') === 'testing' && @rename($validated['tmp_name'], $staging));
    if (!$moved) throw new RuntimeException('Unable to store the uploaded site logo.');
    @chmod($staging, 0644);
    if (!@rename($staging, $destination)) { @unlink($staging); throw new RuntimeException('Unable to activate the uploaded site logo.'); }
    foreach (['png','webp','jpg'] as $extension) {
        $other = $directory . '/site-logo.' . $extension;
        if ($other !== $destination && is_file($other)) @unlink($other);
    }
    clearstatcache(true, $destination);
    $asset = coveted_site_logo_asset();
    if ($asset === null) { @unlink($destination); throw new RuntimeException('The uploaded site logo could not be read back.'); }
    coveted_audit('admin.site_logo_uploaded','site_branding','site_logo',[
        'filename'=>$filename,'mime_type'=>$validated['mime_type'],'width'=>$validated['width'],'height'=>$validated['height'],'bytes'=>$validated['bytes'],
    ],(int)$admin['id']);
    return $asset;
}
...
function coveted_site_logo_delete(array $admin): void
{
    ...
    foreach (['png','webp','jpg'] as $extension) {
        ...
        if (is_file($path) && !@unlink($path)) throw new RuntimeException('Unable to remove the active site logo.');
    }
 ...
 * outside the core operational brain so none becomes a hard dependency.
 *
...
function coveted_site_branding_enrich_agent_snapshot(array $snapshot): array
{
    ...
    $total = count($checks);
    $snapshot['readiness'] = ['ready'=>$ready,'total'=>$total,'percent'=>$total > 0 ? (int)round(($ready / $total) * 100) : 100,'checks'=>$checks];
    $snapshot['branding'] = ['logo_uploaded'=>$logo !== null];

    $opportunities = array_values((array)($snapshot['opportunities'] ?? []));
    if ($logo === null) {
        $opportunities[] = [
            'priority'=>2,'key'=>'site-logo','category'=>'Branding','title'=>'Upload the Coveted site logo',
            'detail'=>'Replace the text-only brand treatment with your uploaded logo across the public header and Admin shell.',
            'href'=>'/admin/branding.php','evidence'=>'No active site logo image is installed.',
        ];
    }
    ...
    try {
        ...
        $opportunities = array_values(array_filter($opportunities, static fn(array $item): bool => (string)($item['key'] ?? '') !== 'crm-pipeline'));

        $followUp = (int)($intelligence['follow_up_due'] ?? 0);
        if ($followUp > 0) $opportunities[] = [
            'priority'=>1,'key'=>'crm-follow-up-due','category'=>'Growth','title'=>'Follow up on active CRM outreach',
            'detail'=>'Contacted CRM records have gone at least three days without a recorded review update.',
            'href'=>'/admin/crm.php?status=contacted','evidence'=>$followUp . ' follow-up' . ($followUp === 1 ? '' : 's') . ' due.',
        ];
        $conversionReady = (int)($intelligence['conversion_ready'] ?? 0);
        if ($conversionReady > 0) $opportunities[] = [
            'priority'=>1,'key'=>'crm-conversion-ready','category'=>'Growth','title'=>'Review qualified CRM records for conversion',
            'detail'=>'Qualified invite records are ready for the System Admin to review against the canonical conversion workflow.',
            'href'=>'/admin/crm.php?status=qualified','evidence'=>$conversionReady . ' qualified record' . ($conversionReady === 1 ? '' : 's') . ' ready for review.',
        ];
        $highPriority = (int)($intelligence['high_priority'] ?? 0);
        if ($highPriority > 0) $opportunities[] = [
            'priority'=>1,'key'=>'crm-high-priority','category'=>'Growth','title'=>'Work high-priority CRM submissions',
            ...
            'href'=>'/admin/crm.php','evidence'=>$highPriority . ' high-priority active record' . ($highPriority === 1 ? '' : 's') . '.',
        ];
        $aging = (int)($intelligence['aging_new'] ?? 0);
        if ($aging > 0) $opportunities[] = [
            'priority'=>2,'key'=>'crm-aging-new','category'=>'Growth','title'=>'Clear aging new CRM submissions',
            'detail'=>'New CRM records have been waiting at least seven days without moving into outreach or qualification.',
            'href'=>'/admin/crm.php?status=new','evidence'=>$aging . ' aging new record' . ($aging === 1 ? '' : 's') . '.',
        ];
    } catch (Throwable $e) {
