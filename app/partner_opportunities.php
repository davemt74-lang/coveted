...
function coveted_partner_opportunities_for_business(array $actor, int $businessId, ?PDO $pdo = null): array
{
    ...
    if (!$business) throw new InvalidArgumentException('Business not found.');
    ...
    foreach ($relationships as $relationship) {
        ...
        if ($upcomingDaily > 0 && $activeCodes < 1) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,1,'restore_checkin','Restore Daily Event check-in readiness','An upcoming Daily Event is assigned here, but this location has no active location or business-wide employee claim code. Restore a canonical claim-code path before members arrive.',$upcomingDaily.' upcoming Daily Event'.($upcomingDaily===1?'':'s').'; 0 active check-in codes.','/business.php?business='.rawurlencode((string)$business['public_id']).'&tab=locations','Manage claim codes');
        }

        if ($status === 'event_venue' && $completedEvents >= 2 && $verifiedVisits >= 8) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,2,'review_partner_status','Review this venue for Partner status','The venue has repeated completed-event history and meaningful verified attendance. Relationship status remains an intentional Admin decision; Coveted recommends the review but never auto-promotes it.',$completedEvents.' completed events; '.$verifiedVisits.' verified visits.',$relationshipHref,'Review relationship');
        } elseif ($status === 'partner' && $completedEvents >= 3 && $repeatAttendees >= 2 && $returnClaims >= 1) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,2,'review_preferred_status','Review this Partner for Preferred Partner status','The relationship has repeat attendance plus verified return activity. Review whether it should become a Preferred Partner; no metric changes relationship status automatically.',$completedEvents.' completed events; '.$repeatAttendees.' repeat attendees; '.$returnClaims.' return-linked claim'.($returnClaims===1?'':'s').'.',$relationshipHref,'Review relationship');
        } elseif ($status === 'preferred_partner' && $completedEvents >= 5 && $repeatAttendees >= 3 && $returnClaims >= 2) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,3,'review_home_venue_status','Review whether this has become a Home Venue relationship','This Preferred Partner has deep repeated usage and return activity. Home Venue remains an explicit relationship decision based on the real group/venue fit, not an automatic score.',$completedEvents.' completed events; '.$repeatAttendees.' repeat attendees; '.$returnClaims.' return-linked claims.',$relationshipHref,'Review relationship');
        }

        if ($benefitsEnabled && $completedEvents >= 1 && $verifiedVisits >= 5 && $upcomingEvents === 0) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,2,'schedule_next_daily','Plan the next Daily Event with this partner',$isSystemAdmin?'This benefit-enabled relationship has proven attendance but no upcoming gathering. Consider another Admin-created Daily Event using the same group × location relationship if timing and reward economics make sense.':'This benefit-enabled relationship has proven attendance but no upcoming gathering. Prepare the location, reward and claim-code readiness for a future Daily Event; Coveted System Admin retains event creation authority.',$verifiedVisits.' verified visits across '.$completedEvents.' completed event'.($completedEvents===1?'':'s').'; no upcoming event.',$isSystemAdmin?'/admin/daily-events.php':'/business-daily-events.php?business='.rawurlencode((string)$business['public_id']),$isSystemAdmin?'Plan Daily Event':'Review Daily Events');
        }
        ...
        if ($completedDaily >= 2 && $thresholdTotal > 0 && $thresholdsUnlocked >= $completedDaily && $dailyVerified >= (int)ceil($thresholdTotal * 1.35)) {
            ...
            coveted_partner_opportunity_add($items,$businessId,$relationship,3,'raise_future_threshold','Consider a higher threshold on the next Daily Event','Recent Daily Events consistently cleared their group reward thresholds by a wide margin. Review a higher threshold for the next event rather than changing a completed event or retroactively altering earned rewards.',$completedDaily.' completed Daily Events; average '.$averageVerified.' verified vs. '.$averageThreshold.' threshold; all observed thresholds unlocked.',$isSystemAdmin?'/admin/daily-events.php':$relationshipHref,$isSystemAdmin?'Plan next threshold':'Review performance');
        }

        if ($benefitsEnabled && $verifiedVisits >= 5 && $returnClaims === 0 && $activeReturnCampaigns === 0) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,2,'create_return_value','Add a return-visit value layer','This relationship has delivered verified visits but has no observed return-linked claims and no non-archived Business return-visit or guest-return campaign at this location. Consider a return Benefit Program after reviewing partner economics.',$verifiedVisits.' verified visits; 0 return-linked claims; 0 return-visit campaigns for this location.',$isSystemAdmin?'/admin/benefit-programs.php':'/business.php?business='.rawurlencode((string)$business['public_id']).'&tab=campaigns',$isSystemAdmin?'Review Benefit Programs':'Review campaigns');
        }
        ...
        if (in_array($status,['partner','preferred_partner','home_venue'],true) && $completedEvents>0 && $upcomingEvents===0 && $lastCompletedTs!==false && $lastCompletedTs<time()-(45*86400)) {
            $days=max(45,(int)floor((time()-$lastCompletedTs)/86400));
            coveted_partner_opportunity_add($items,$businessId,$relationship,2,'reengage_dormant','Re-engage this established partner relationship','The venue is an established partner with successful history but no upcoming event. Review the relationship before the historical momentum goes cold.',$completedEvents.' completed events; last completed gathering was about '.$days.' days ago; no upcoming event.',$relationshipHref,'Open relationship');
        }

        if ($completedDaily > 0 && $dailyRewards >= 5 && $claims === 0) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,3,'review_reward_use','Review post-event reward use','Daily Event group rewards have been issued here but this relationship currently shows no verified claims. Check reward fit, expiration, claim instructions and return timing before repeating the same offer.',$dailyRewards.' Daily Event reward issuances; 0 relationship claims.','/business-daily-events.php?business='.rawurlencode((string)$business['public_id']),'Review Daily Event rewards');
        }

        if ($dailyEvents > 0 && $upcomingDaily === 0 && $activeCodes < 1) {
            coveted_partner_opportunity_add($items,$businessId,$relationship,3,'prepare_future_checkin','Prepare claim-code readiness before the next Daily Event','This location has Daily Event history but no active check-in code today. Restore a location or business-wide employee claim code before another Daily Event is assigned.',$dailyEvents.' Daily Event'.($dailyEvents===1?'':'s').' in relationship history; 0 active check-in codes.','/business.php?business='.rawurlencode((string)$business['public_id']).'&tab=locations','Manage claim codes');
        }
    }

    $deduped=[];foreach($items as $item)$deduped[(string)$item['key']]=$item;$items=array_values($deduped);
    usort($items,static function(array $a,array $b):int{$priority=(int)$a['priority']<=>(int)$b['priority'];return $priority!==0?$priority:strcmp((string)$a['key'],(string)$b['key']);});
    $counts=['total'=>count($items),'p1'=>0,'p2'=>0,'p3'=>0];foreach($items as $item)$counts['p'.max(1,min(3,(int)$item['priority']))]++;
    return [
        'generated_at'=>gmdate('Y-m-d H:i:s'),'business_ref'=>(string)$business['public_id'],'business_name'=>(string)$business['name'],
        ...
        'counts'=>$counts,'recommendations'=>array_slice($items,0,24),
    ];
}

/**
 * Partner intelligence supplied to the Admin Agent. Partner CRM is merged here
 * so Agent Chat, the proactive briefing and Partner Opportunities all share one
 * canonical read-only context rather than maintaining a second CRM brain.
 */
function coveted_partner_opportunities_agent_context(array $admin, int $limit = 12, ?PDO $pdo = null): array
{
    ...
    foreach(array_slice(coveted_businesses_for_actor($admin),0,50) as $business){
        try{
            ...
            foreach((array)$snapshot['recommendations'] as $item)if(is_array($item))$recommendations[]=$item;
        }catch(Throwable $e){
    ...
    $partnerCrm=['unavailable'=>true,'counts'=>[],'relationships'=>[],'recent_interactions'=>[],'recommendations'=>[]];
    try {
    ...
    $deduped=[];foreach($recommendations as $item){$key=trim((string)($item['key']??''));if($key!=='')$deduped[$key]=$item;}
    $recommendations=array_values($deduped);
    usort($recommendations,static function(array $a,array $b):int{$priority=(int)($a['priority']??3)<=>(int)($b['priority']??3);return $priority!==0?$priority:strcmp((string)($a['key']??''),(string)($b['key']??''));});
    ...
    return [
        ...
        'privacy'=>'Aggregate partner relationship intelligence plus compact Partner CRM context. Contact names/roles and interaction summaries may be supplied to the Admin Agent; raw partner email and phone values are intentionally excluded from broad LLM context. No member identities or private Loyalty balances are exposed.',
        'action_policy'=>'Read-only recommendations. The Admin Agent may explain and prioritize partner opportunities and CRM follow-ups but must not treat them as authorization to mutate contacts, notes, follow-ups, relationship state, events, rewards or campaigns.',
        'counts'=>$counts,
        'crm'=>$partnerCrm,
